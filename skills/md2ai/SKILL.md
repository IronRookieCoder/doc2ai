...
- 处理完成后删除 `manifest.json`、`risk-index.json` 和 `summary.json`。
...
6. 核实完成后删除 `manifest.json`、`risk-index.json` 和输出根目录下的 `summary.json`。
...
- 最终交付目录中没有 `manifest.json`、`risk-index.json` 和 `summary.json`。
- 如处理期间发现 `risk_count > 0`，已经按风险索引做精准核实或汇总人工关注项，然后再删除过程文件。
- 中文路径、中文目录、中文文件名保持可读。
